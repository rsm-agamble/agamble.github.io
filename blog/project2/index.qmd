...
<!-- ### Check Results with statsmodels GLM (Poisson Regression)

```{python}
import statsmodels.api as sm
import numpy as np

# Drop your manual intercept and let statsmodels add it
X_sm = X.drop(columns='intercept', errors='ignore')

# Make sure all columns are numeric
X_sm = X_sm.apply(pd.to_numeric)

# Add constant (intercept)
X_sm = sm.add_constant(X_sm)

# Response variable
y_sm = pd.to_numeric(blueprinty['patents'])

# Fit Poisson GLM
model = sm.GLM(y_sm, X_sm, family=sm.families.Poisson())
results = model.fit()

# Summary of coefficients and standard errors
